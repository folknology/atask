...
impl KanbanColumn {
    ...
    pub fn remove_card(&mut self, issue_number: u64) -> Option<KanbanCard> {
        if let Some(pos) = self
            .cards
            .iter()
            .position(|card| card.issue_number == issue_number)
        {
...
impl KanbanCard {
    ...
    pub fn set_priority_from_labels(&mut self) {
        self.priority = if self
            .labels
            .iter()
            .any(|l| l.contains("critical") || l.contains("Critical"))
        {
            Priority::Critical
        } else if self
            .labels
            .iter()
            .any(|l| l.contains("high") || l.contains("High"))
        {
            Priority::High
        } else if self
            .labels
            .iter()
            .any(|l| l.contains("low") || l.contains("Low"))
        {
            Priority::Low
        } else {
            Priority::Medium
...
impl KanbanService {
        ...
        for issue in all_issues {
            ...
            for column in &mut board.columns {
                if column.id != "backlog"
                    && !column.label_name.is_empty()
                    && issue
                        .labels
                        .iter()
                        .any(|label| label.name == column.label_name)
                {
            ...
            if !placed {
                if let Some(backlog_column) =
                    board.columns.iter_mut().find(|col| col.id == "backlog")
                {
    ...
    pub async fn move_issue(
        ...
        if !from_column.is_empty() && from_column != "backlog" {
            let _ = self
                .github_ops
                .remove_label_from_issue(issue_number, from_column)
                .await;
        }
        ...
        if !to_column.is_empty() && to_column != "backlog" {
            self.github_ops
                .add_label_to_issue(issue_number, to_column)
                .await?;
        }
        ...
        let filtered_issues = all_issues
            ...
            .filter(|issue| {
                issue
                    .labels
                    .iter()
                    .any(|issue_label| issue_label.name == label)
            })
            .collect();
        ...
        let backlog_issues = all_issues
            ...
            .filter(|issue| {
                ...
                !issue
                    .labels
                    .iter()
                    .any(|label| workflow_labels.contains(&label.name.as_str()))
            })
            .collect();
...
mod tests {
    ...
    fn test_kanban_board_creation() {
        ...
        assert_eq!(
            column_names,
            vec!["Preparing", "Progressing", "Done", "Backlog"]
        );
    }
    ...
    fn test_serialization() {
        ...
        assert!(
            deserialized.is_ok(),
            "KanbanBoard should be deserializable from JSON"
        );
