...
use git2::{Commit as Git2Commit, Oid, Repository, Time};
...
impl GitOps {
    ...
    pub fn new() -> Result<Self> {
        let repo =
            Repository::open(".").context("Failed to open Git repository in current directory")?;
        Ok(Self { repo })
    }
    ...
    pub fn new_from_path<P: AsRef<Path>>(path: P) -> Result<Self> {
        let repo =
            Repository::open(path).context("Failed to open Git repository at specified path")?;
        Ok(Self { repo })
    }
    ...
    pub fn get_commits(&self, limit: Option<usize>) -> Result<Vec<CommitInfo>> {
        ...
        for (count, commit_id) in revwalk.enumerate() {
    ...
    pub fn get_commit_by_hash(&self, hash: &str) -> Result<Option<CommitInfo>> {
        let oid = Oid::from_str(hash).context("Invalid commit hash format")?;
    ...
    fn convert_commit_to_info(&self, commit: &Git2Commit) -> Result<CommitInfo> {
        ...
        Ok(CommitInfo {
            ...
            commit_date,
            message: commit.message().unwrap_or("No message").trim().to_string(),
            files_changed,
            insertions,
            deletions,
        })
...
mod tests {
    ...
    fn test_git_ops_creation_current_directory() {
        ...
        assert!(
            result.is_ok(),
            "Should be able to create GitOps in current git repository"
        );
    }
    ...
    fn test_git_ops_creation_from_path() {
        ...
        match result {
            ...
            Err(_) => {
                ...
                println!(
                    "Warning: Could not create test git repository - git may not be available"
                );
            }
        }
    }
    ...
    fn test_git_ops_creation_invalid_path() {
        ...
        assert!(
            result.is_err(),
            "Should fail to create GitOps for non-existent path"
        );
    }
    ...
    fn test_get_commit_by_nonexistent_hash() {
        if let Ok((_temp_dir, git_ops)) = create_test_git_repo() {
            ...
            assert!(
                commit.is_none(),
                "Should return None for non-existent commit"
            );
        }
    }
    ...
    fn test_commit_info_serialization() {
        ...
        assert!(
            deserialized.is_ok(),
            "CommitInfo should be deserializable from JSON"
        );
    ...
    fn test_issue_params_serialization() {
        ...
        assert!(
            deserialized.is_ok(),
            "IssueParams should be deserializable from JSON"
        );
    ...
    async fn test_github_ops_creation_with_token() {
        ...
        assert!(
            result.is_ok(),
            "Should create GitHubOps with valid parameters"
        );
    }
    ...
    async fn test_github_ops_creation_empty_token() {
        ...
        assert!(
            result.is_ok(),
            "Should create GitHubOps even with empty token"
        );
    }
    ...
    async fn test_github_ops_from_env_with_token() {
        ...
        let result = GitHubOps::from_env("test_owner".to_string(), "test_repo".to_string());
    ...
    async fn test_github_ops_with_real_token() {
        ...
        if let Ok(token) = std::env::var("GITHUB_TOKEN") {
            let result = GitHubOps::new(token, "folknology".to_string(), "atask".to_string());
    ...
    async fn test_add_label_to_issue_method_exists() {
        ...
        assert!(
            result.is_err(),
            "Should fail with authentication error for fake token"
        );
    ...
    async fn test_replace_issue_labels_method_exists() {
        ...
        // This will fail with authentication error, but method is implemented
        let result = github_ops
            .replace_issue_labels(1, vec!["label1", "label2"])
            .await;
        assert!(
            result.is_err(),
            "Should fail with authentication error for fake token"
